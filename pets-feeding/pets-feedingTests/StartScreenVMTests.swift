...
class StartScreenVMTests: XCTestCase {
	...
	func testChesckStausCalendar() {
		...
		testsClassExample.gettingCalendarStatusInterlayer(testStatus, completion: { status in
			XCTAssertEqual(status, expectedValue, "gettingCalendarStatusInterlayer() ERROR!, tested status: \(testStatus), result expected: \(expectedValue), returned from func.: \(status)")
		})
